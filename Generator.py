...
class Ui_keyGeneration(QWidget):
    ...
    def build(self):
        ...
        fout = open("cipher_APP.py", "wt")
        for line in fin:
            fout.write(line.replace('N_MOD',str(N)))
        fin.close()
        os.remove("out2.txt")
        fout.close()
        QMessageBox.about(self, "Created", "cipher_APP, decipher_App files created ")
